...
class ControlDispatcher:
    def __init__(self, window):
        ...
        self.stopped = True
        self.mining = False
        self.camera_queue = Queue(1)
        self.movement_queue = Queue(1)
        self.jump_queue = Queue(1)
        self.tool_control_queue = Queue(1)
        self.t1: Thread = None
        self.t2: Thread = None
        self.t3: Thread = None
        self.t4: Thread = None
    ...
    def request_movement(self, function_ptr):
        ...
        """
        if (
                self.tool_control_queue.empty()
                and self.camera_queue.empty()
                and not self.mining
                and not self.block
        ):
    ...
    def request_tool_event(self, function_ptr):
        ...
        :param function_ptr: Rotate Function
        """
        if self.tool_control_queue.empty() and not self.mining and not self.block:
            self.tool_control_queue.put(function_ptr)

    def __update_position(self):
        while not self.stopped:
            if not self.movement_queue.empty() and not self.mining and not self.block:
    ...
    def __update_jump(self):
        while not self.stopped:
            if not self.jump_queue.empty() and not self.mining and not self.block:
                self.jump_queue.get()()
    ...
    def __update_rotate(self):
        while not self.stopped:
            if not self.camera_queue.empty() and not self.mining and not self.block:
    ...
    def __update_tool(self):
        while not self.stopped:
            if (
                    not self.tool_control_queue.empty()
                    and not self.mining
                    and not self.block
            ):
